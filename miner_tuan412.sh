...
for ((i = 0; i < $gpus; i++)); do
    ...
    if [ $i -eq 0 ]; then
        screen -S "ipad" -dm bash -c "$command"
    else
        screen -S "ipad" -X screen bash -c "$command"
    fi
done
if [ $gpus -gt 0 ]; then
...
if $silence; then
    ...
    echo "If you want to stop, run: pkill tuan412 && pkill -f submitminer. or simply use pkill -f miner"
else
    bash -c "$command"
    pkill -f "submitminer"
    pkill ipad
